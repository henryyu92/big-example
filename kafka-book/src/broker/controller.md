...
Kafka 集群会选举一个 broker 作为 KafkaController，每个 Broker 在启动时会监听 `ZooKeeper` 的 `/controller` 结点，如果不存在则说明尚未选举出 Controller，此时 broker 会尝试创建 `/controller` 结点，如果创建成功则该 broker 成为 Controller。
...
- `ReassignedPartitionLeaderSelector`：将 ISR 中的第一个副本作为 leader
- `PreferredReplicaPartitionLeaderSelector`：从 assignedReplicas 取出的第一个副本

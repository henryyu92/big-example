...
public class DiskStore implements Closeable {
    ...
    public MStore.SeekIter<KeyValue> createIterator(List<DiskFile> diskFiles) throws IOException {
        List<MStore.SeekIter<KeyValue>> iters = new ArrayList<>();
    ...
    public static class DefaultFlusher implements MiniBase.Flusher {
        ...
        @Override
        public void flush(MiniBase.Iter<KeyValue> it) throws IOException {
    ...
    public static class DefaultCompactor extends MiniBase.Compactor {
        private DiskStore diskStore;
        ...
        private void performCompact(List<DiskFile> filesToCompact) throws IOException {
            ...
            try {
                try (DiskFile.DiskFileWriter writer = new DiskFile.DiskFileWriter(fileTempName)) {
                    for (MiniBase.Iter<KeyValue> it = diskStore.createIterator(filesToCompact); it.hasNext();) {
                        writer.append(it.next());
                    }
                    writer.appendIndex();
                    writer.appendTrailer();
    ...
    public static class MultiIter implements MStore.SeekIter<KeyValue> {

        private class IterNode {
            KeyValue kv;
            MStore.SeekIter<KeyValue> iter;

            public IterNode(KeyValue kv, MStore.SeekIter<KeyValue> it) {
                this.kv = kv;
                this.iter = it;
            }
        }

        private MStore.SeekIter<KeyValue> iters[];
        private PriorityQueue<IterNode> queue;

        public MultiIter(MStore.SeekIter<KeyValue> iters[]) throws IOException {
            assert iters != null;
        ...
        public MultiIter(List<MStore.SeekIter<KeyValue>> iters) throws IOException {
            this(iters.toArray(new MStore.SeekIter[0]));
        }
        ...
        public void seekTo(KeyValue kv) throws IOException {
            queue.clear();
            for (MStore.SeekIter<KeyValue> it : iters) {
                it.seekTo(kv);
                if (it.hasNext()) {

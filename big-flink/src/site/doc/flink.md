### Flink 编程模型
Flink 通过流处理模型来统一处理不同类型的数据集，将有界数据转换为无界数据从而将批处理和流处理统一在一套流式引擎中。
```scala
def main(args: Array[String]): Unit = {
    ...
    val counts = text
      ...
      .sum(1)
    // 结果输出
    counts.writeAsCsv("file:///path/result")
    // 触发提交
...
#### Flink 执行环境  
Flink 执行环境决定了程序运行的环境(本地或者集群)，也决定了应用的类型。```StreamExecutionEnvironment``` 是流式数据处理环境，```ExecutionEnvironment``` 是批数据处理环境。有三种方式获取 Flink 的执行环境：
```scala
// 流处理运行环境创建

// 如果在本地运行则创建本地环境，在集群启动则创建集群环境
StreamExecutionEnvironment.getExecutionEnvironment()
// 创建本地运行环境并指定并行度
StreamExecutionEnvironment.createLocalEnvironment(parallelism: Int)
// 指定远程 JobManager 的 IP 和端口以及程序的 Jar 包和依赖
StreamExecutionEnvironment.createRemoteEnvironment(host: String, port: Int, jarFiles: String*)

// 批处理运行环境创建

// 如果在本地启动则创建本地环境，在集群启动则创建集群环境
ExecutionEnvironment.getExecutionEnvironment
// 创建本地运行环境并指定并行度
ExecutionEnvironment.createLocalEnvironment(parallelism: Int)
// 指定远程 JobManager 的 IP 和端口以及程序的 Jar 包和依赖
ExecutionEnvironment.createLocalEnvironment(host: String, port: Int, jarFiles: String*)
```
...
```
完成对 Table 的注册在内部 CataLog 生成相应的数据表信息就可以使用 SQL 语句对表进行处理完成数据转换操作
### Flink 监控调优
### Flink & Kafka